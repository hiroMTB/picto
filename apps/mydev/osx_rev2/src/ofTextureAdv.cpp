...
//	glDisable(GL_TEXTURE_2D);
    ...
    glBindTexture(GL_TEXTURE_RECTANGLE_ARB, (GLuint)texName[0]);
    // upload 0 or white pixels....
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, internalGlDataType, tex_w, tex_h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0);  // init to black...
...
void ofTextureAdv::setExternalTexture(int _w, int _h, GLuint _texName){
    texName[0] = _texName;
    ...
    tex_t = (float)(_w) / (float)tex_w;
	tex_u = (float)(_h) / (float)tex_h;
...
void ofTextureAdv::updatePoints(){
	...
	for(int y = 0; y < gridSizeY; y++){	
		for(int x = 0; x < gridSizeX; x++){
			...
			coor[y*gridSizeX + x].set(tt*tex_w, tex_h*((tex_u - offseth) - uu), 0);
		}
	}
